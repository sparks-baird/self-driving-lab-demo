...
class MQTTClient(simple2.MQTTClient):
    ...
    def cbstat(A, pid, stat):
        ...
        for (B, C) in A.msg_to_confirm.items():
            if D in C:
                if E == 0:
                    if B not in A.msg_to_send:
                        A.msg_to_send.insert(0, B)
                    C.remove(D)
                    if not C:
                        A.msg_to_confirm.pop(B)
                elif E in (1, 2):
                    A.msg_to_confirm.pop(B)
                return
        for (B, C) in A.sub_to_confirm.items():
            if D in C:
                if E == 0:
                    if B not in A.sub_to_send:
                        A.sub_to_send.append(B)
                    C.remove(D)
                    if not C:
                        A.sub_to_confirm.pop(B)
                elif E in (1, 2):
                    A.sub_to_confirm.pop(B)
    ...
    def add_msg_to_send(A, data):
        C = len(A.msg_to_send)
        C += sum(map(len, A.msg_to_confirm.values()))
        while C >= A.MSG_QUEUE_MAX:
            E = min(
                map(lambda x: x[0] if x else 65535, A.msg_to_confirm.values()),
                default=0,
            )
            if 0 < E < 65535:
                B = None
                for (F, D) in A.msg_to_confirm.items():
                    if D and D[0] == E:
                        ...
                        B = F
                        break
                if B and B in A.msg_to_confirm and not A.msg_to_confirm[B]:
                    A.msg_to_confirm.pop(B)
            else:
                A.msg_to_send.pop(0)
            C -= 1
    ...
    def publish(A, topic, msg, retain=False, qos=0):
        ...
        if D:
            A.msg_to_send[:] = [
                B for B in A.msg_to_send if not (E == B[0] and D == B[2])
            ]
        try:
            F = super().publish(E, msg, D, B, False)
            if B == 1:
                A.msg_to_confirm.setdefault(C, []).append(F)
                if len(A.msg_to_confirm[C]) > A.CONFIRM_QUEUE_MAX:
                    A.msg_to_confirm.pop(0)
            return F
        except (OSError, simple2.MQTTException) as G:
            A.conn_issue = G, 2
            if A.NO_QUEUE_DUPS:
                if C in A.msg_to_send:
                    return
    ...
    def subscribe(A, topic, qos=0, resubscribe=True):
        ...
        if A.RESUBSCRIBE and resubscribe:
            if C not in dict(A.subs):
                A.subs.append(B)
        A.sub_to_send[:] = [B for B in A.sub_to_send if C != B[0]]
        try:
            D = super().subscribe(C, qos)
            A.sub_to_confirm.setdefault(B, []).append(D)
            if len(A.sub_to_confirm[B]) > A.CONFIRM_QUEUE_MAX:
                A.sub_to_confirm.pop(0)
            return D
        except (OSError, simple2.MQTTException) as E:
            A.conn_issue = E, 3
            if A.NO_QUEUE_DUPS:
                if B in A.sub_to_send:
                    return
            A.sub_to_send.append(B)
